function [X,N,I,p,M]=d1fn3veloce(petot,nutot) %d of generator given as permutation + series of 0 and 1, to start

%tutto va messo in una matrice 
...
n=size(petot,2);
if not(size(nutot,2)==n-1)
    disp('error')
return
end

k=sum(nutot(1,:),2); %dimensione
basis=partfox(k,n,2); %base di fox   %per dimensione k con n+1 punti (occhio quindi n spazi) in R^3
ba=size(basis,1);

M(factorial(n+1),ba)=uint8(0); %matricione


    for ipe=1:size(petot,1)
ipe
pe=petot(ipe,:);
nu=nutot(ipe,:);
...
%parte facile: d_0
X1=[1 pe+1];  %devo riconoscere
np=numeroperm(X1);
%devo riconoscere anche questo
N1=[0 nu]; 
for iba=1:ba
    if isequal(N1,basis(iba,:))
        break; end
end
    M(np,iba)=xor(M(np,iba),1);  %aggiungo 1 mod 2
